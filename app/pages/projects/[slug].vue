<script setup lang="ts">
  ...
  const project = computed(() => getProjectWithSkills(slug.value, lang.value));
  ...
  useSeoMeta({
    title: () => project.value?.name ?? t('project.fallbackTitle'),
  ...
  const siblings = computed(() => {
    const all = getProjects(lang.value);
...
<template>
  <div v-if="project" class="flex flex-col gap-16">
    <div class="flex flex-col gap-5 py-6">
      ...
      >
        <Icon name="lucide:arrow-left" size="16" /> {{ t('actions.backToWork') }}
      </NuxtLink>
    ...
    <section v-if="project.links && project.links.length" class="flex flex-col gap-3">
      <SectionHeading :eyebrow="t('project.linksEyebrow')" :title="t('project.linksTitle')" />
    ...
    <section v-if="project.skills && project.skills.length" class="flex flex-col gap-3">
      <SectionHeading :eyebrow="t('project.stackEyebrow')" :title="t('project.stackTitle')" />
    ...
    <nav
      ...
      :aria-label="t('project.pagination')"
      class="flex justify-between gap-4 border-t pt-6 text-sm"
    >
      <NuxtLink
        v-if="siblings.prev"
        :to="localePath(`/projects/${siblings.prev.slug}`)"
      ...
      <NuxtLink
        v-if="siblings.next"
        :to="localePath(`/projects/${siblings.next.slug}`)"
